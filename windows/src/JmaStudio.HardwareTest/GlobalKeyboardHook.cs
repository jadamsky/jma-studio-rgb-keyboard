// A minimal WH_KEYBOARD_LL global keyboard hook, for interactively
// testing key-state-driven effects (currently just typing_reactive)
// against REAL keystrokes instead of a synthetic simulated press.
//
// This is test tooling living in JmaStudio.HardwareTest, not part of
// JmaStudio.Hardware or a real input-listener component -- the eventual
// Windows Service (Phase 5) will need its own equivalent of Python's
// daemon/input_listener.py, and this class (plus WindowsKeyMap.cs) is a
// reasonable starting point for that when the time comes, but hasn't
// been hardened for production use (no thread-safe multi-consumer
// story beyond a single event, no graceful unhook-on-Ctrl+C, etc.).
//
// Same narrow-scope principle as the Python original: this only ever
// produces a key NAME on keydown, nothing about content, and nothing
// persisted anywhere.
